...
def ask_claude(prompt: str) -> str:
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "Check GitHub secret: Settings → Secrets → Actions → ANTHROPIC_API_KEY. "
            "The value must be the full key starting with 'sk-ant-'."
        )
    log(f"API key found: {api_key[:10]}...{api_key[-4:]} (length: {len(api_key)})")
 
    if not api_key.startswith("sk-ant-"):
        raise RuntimeError(
            f"ANTHROPIC_API_KEY doesn't start with 'sk-ant-'. "
            f"Got: '{api_key[:15]}...' — this is not a valid Anthropic API key. "
            f"Go to console.anthropic.com → API Keys → copy the full key."
        )
